...
class SubstancePainterSessionCollector(HookBaseClass):
        ...
        for texture_set_name, texture_set in map_export_info.items():
            for texture_id, texture_file in texture_set.items():
                if os.path.exists(texture_file):
                    ...
                    textures_item = super(SubstancePainterSessionCollector, self).process_file(settings, parent_item, texture_file)
                    if textures_item:
                        textures_item.context = parent_item.context
                        textures_item.type = "substancepainter.texture"
                        textures_item.type_display = "Texture"
    ...
    def collect_current_substancepainter_session(self, settings, parent_item):
        ...
        if path:
            ...
            session_info = {
                "item_type":"substancepainter.session",
                "type_display":"Substance Painter Session",
                "display_name":file_info["filename"]
                }         
            session_item = super(SubstancePainterSessionCollector, self).process_file(settings, parent_item, path, custom_info=session_info)
