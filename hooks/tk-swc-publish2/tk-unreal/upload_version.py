...
class UploadVersionPlugin(HookBaseClass):
    ...
    def validate(self, settings, item):
        ...
        if "versionless_publish_name" in item.properties:
            self.logger.info("item versionless publish_name is   : %s." % item.properties["versionless_publish_name"])
        if "publish_name" in item.properties:
            self.logger.info("item publish_name is   : %s." % item.properties["publish_name"])
        if "publish_version" in item.properties:
            self.logger.info("item publish_version is  : %s." % item.properties["publish_version"])
    ...
    def publish(self, settings, item):
        ...
        item.properties["original_path"] = item.properties["path"]
    ...
    def get_next_version_name(self, item, path):
        ...
        self.logger.info("item properties: %s" % item.properties)
        self.logger.info("path: %s" % path)
        self.logger.info("original publish name: %s" % publish_name)
        self.logger.info("project name: %s" % project_name)
        self.logger.info("description: %s" % description)
        self.logger.info("entity name: %s" % entity_name)
        ...
        if entity_name and task_name:
            publish_name = "{}_{}_{}".format(project_name, entity_name, task_name)
        else:
            publish_name = "{}".format(project_name)
        if description:
            publish_name = "{}_{}".format(publish_name, description)
        self.logger.info("updated publish name: %s" % publish_name)
        ...
        self.logger.info("version_number: %s" % version_number)

        base_name = os.path.basename(path)
        dir_name = os.path.dirname(path)
        (base_name, ext) = os.path.splitext(base_name)
        path = "{}\\{}{}".format(dir_name, publish_name, ext)

        version_path = self.publisher.util.get_version_path(path,version_number)
        self.logger.info("version_path: %s" % version_path)
        return version_path
