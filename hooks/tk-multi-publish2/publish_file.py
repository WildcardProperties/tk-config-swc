...
class PublishPlugin(HookBaseClass):
    ...
    def validate(self, settings, item):
        ...
        if not item.context.entity and not target_context.entity:
            ...
            elif not item.context.task:
                if not item.local_properties.get("ignore_bad_stuff", False):
                    self.logger.warning("Please set a Task for this Published File!", extra={
                        "action_button":{
                        "label": "Ignore",
                        "tooltip": "Ignore this warning",
                        "callback": self._ignore_warning,
    ...
    def finalize(self, settings, item):
        ...
        # This doesn't work... todo figure out how to remove temp thumbnails somewhere. 
        # if publish_data["thumbnail"]:
        #     if os.path.exists(publish_data["thumbnail"]) and os.path.dirname(publish_data["thumbnail"]) == tempfile.gettempdir():
        #         os.remove(publish_data["thumbnail"])
    ...
    def ensure_path(self, item):
        p4_data = item.properties.get("p4_data")
        if p4_data: 
            return p4_data["clientFile"]
        else:
