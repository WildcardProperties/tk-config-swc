...
class BasicSceneCollector(HookBaseClass):
    ...
    def common_file_info(self):
        ...
        if not hasattr(self, "_common_file_info"):
            ...
            self._common_file_info = {
                ...
                "Audio Files": {
                    "extensions": ["wav", "mp3"],
                    "icon": self._get_icon_path("audio.png"),
    ...
    def common_folder_info(self):
        ...
        if not hasattr(self, "_common_folder_info"):
            ...
            self._common_folder_info = {
                ...
                "Review": {
                    "name": "review",
                    "item_type": "folder.review",
                    "ignored": False,
                },                               
                "Houdini Backup": {
                    "name": "backup",
                    "item_type": "folder.houdini.backup",
                    "ignored": True,
                },
                "Local WIP Files": {
                    "name": "ignore",
                    "item_type": "folder.ignore",
                    "ignored": True,
                },         
                "Audio Mixdown": {
                    "name": "Mixdown",
    ...
    def _collect_file(self, parent_item, item_info, frame_sequence=False):
        ...
        if os.path.basename(os.path.dirname(item_info["item_path"])) == "review" and item_info["item_type"]:
            item_type = "review.%s" % item_info["item_type"].split(".")[1]
        else:
        ...
        elif item_type.endswith("file.audio"):
            try:
                temp_path = os.path.expandvars(r'%APPDATA%\Shotgun\Temp')
                os.makedirs(temp_path, exist_ok=True)
                out_path = os.path.join(temp_path,file_item.name.split(".")[0] + ".jpg")                
                input_path = path.replace("\\","/")
                encoded_thumbnail = out_path.replace("\\","/")
                # TODO: Make this program call more abstract in both location and OS
                subprocess.check_output(f'b:/Tools/Programs/ffmpeg/bin/ffmpeg.exe -y -i "{input_path}" -filter_complex "showwavespic=s=640x240:split_channels=1" -frames:v 1 "{encoded_thumbnail}"', shell=True, env={'PATH': os.getenv('PATH')})
                file_item.set_thumbnail_from_path(out_path)
                file_item.set_icon_from_path(out_path)
                ...
                self.logger.info("Audio thumbnail conversion error.")            
        ...
        for child in item_info['children']:
            if os.path.basename(os.path.dirname(child["item_path"])) == "review":
                child_item = self._collect_review(file_item, child)
            else:
    ...
    def _collect_folder(self, parent_item, folder):
        ...
        for items in folder.add_info, folder.edit_info, folder.delete_info, folder.open_info:
            for item in items:
                ...
                if not skip:        
                    item_infos.append(self._collect_item_info(parent_item,item_path,extra=None))
        
        item_infos = self._process_hierarchy(list(item_infos),parent_item)

        for item_info in item_infos:
            # Process each file we find
            if os.path.basename(os.path.dirname(item_info["item_path"])) == "review":
                file_items.append(self._collect_review(parent_item,item_info))
            else:
    ...
    def _collect_single_version(self, parent_item, item_info):
        ...
        # type would be.
        item_info["item_type"] = f"review.{item_info['item_type'].split('.')[1]}"
    ...
    def _collect_review(self, parent_item, item_info):
        """
        Creates items for quicktime playblasts.
    ...
    # def create_folder_item(self, path, parent_item):
    #     # More than one cache, we group the caches in the UI
    #     folder_item_type = "folder"
    #     folder_display_name = path
    #     folder_type_display = "Sub-folder"
        
    #     # create the item
    #     folder_item = parent_item.create_item(folder_item_type, folder_display_name, folder_type_display)
    #     folder_item.expanded = False
    #     # folder_item.set_icon_from_path(os.path.join(self.disk_location, os.pardir, "icons", "maya_multi_set.png"))
